...
// Map location names to the hardcoded paths in App.jsx
const locationRoutes = [
    { name: 'Echo Lounge', path: '/echolounge' },
    { name: 'House of Blues', path: '/houseofblues' },
    { name: 'Pavilion', path: '/pavilion' },
    { name: 'American Airlines Center', path: '/americanairlines' }
]
...
const Locations = () => {
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                // Map fetched data to hardcoded venueNames state for original component structure
                setVenueNames({
    ...
    const getPathForVenue = (venueName) => {
        return locationRoutes.find(route => route.name === venueName)?.path || '/'
    }
    ...
    return (
        <div className='available-locations'>
            {/* The buttons are now Links for navigation */}
            <div id='venue1button' className='venue1-button-overlay'>
                <Link role='button' to={getPathForVenue(venueNames.venue1)}>{venueNames.venue1}</Link>
            </div>

            <div id='venue2button' className='venue2-button-overlay'>
                <Link role='button' to={getPathForVenue(venueNames.venue2)}>{venueNames.venue2}</Link>
            </div>

            <div id='venue3button' className='venue3-button-overlay'>
                <Link role='button' to={getPathForVenue(venueNames.venue3)}>{venueNames.venue3}</Link>
            </div>

            <div id='venue4button' className='venue4-button-overlay'>
                <Link role='button' to={getPathForVenue(venueNames.venue4)}>{venueNames.venue4}</Link>
            </div>
            ...
            <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 1000.32 500" xmlSpace="preserve">
                ...
                {/* Update hrefs to match App.jsx routes */}
                <a href={getPathForVenue(venueNames.venue1)}><polygon id="venue1" points="2.97,234.52 17.94,198.9 34.45,188.58 52.52,191.68 56.65,196.32 69.03,162.26 84,137.48 
                ...
                <a href={getPathForVenue(venueNames.venue2)}><polygon id="venue2" name='venue2' value={2} points="358.58,353.74 376.65,322.77 389.55,314.52 384.39,280.45 407.61,272.19 422.06,220.58 
                ...
                <a href={getPathForVenue(venueNames.venue3)}><polygon id="venue3" name='venue3' value={3} points="998.06,83.81 952.65,31.16 914.45,16.71 877.29,43.55 833.94,102.39 811.74,161.23 
                796.77,241.23 802.97,303.16 833.94,353.23 871.61,385.23 954.71,385.23 1000.32,387.81 " /></a>

                <a href={getPathForVenue(venueNames.venue4)}><polygon id="venue4" name='venue4' value={4} points="625,291 615,305 608,318 625,338 637,354 622.5,358 673,363.5 751,363.5 793,363.5 
